...
AGG_LOG_SETUP(agg_web2, WenChuangSearchProcessor);
AGG_LOG_SETUP_WITH_NAME(displayLog, displayLog, WenChuangSearchProcessor);
...
BEGIN_DEFINE_SERIALIZATION_FUNC(WenChuangSearchProcessor)
...
END_DEFINE_SERIALIZATION_FUNC(WenChuangSearchProcessor)

bool WenChuangSearchProcessor::DoInit(const String2String &params)
{
    ...
    AGG_LOG(INFO, "DoInit WenChuangSearchProcessor Succ.");
    return true;
}

bool WenChuangSearchProcessor::RegisterMethods() {
    return true;
}

BaseProcessor* WenChuangSearchProcessor::DoClone() {
    return new WenChuangSearchProcessor(*this);
}

void WenChuangSearchProcessor::DoDestroy() {
    delete this;
}
...
int WenChuangSearchProcessor::DoProcess(AggContext * aggContext)
{
...
AggWebResultPtr WenChuangSearchProcessor::GetQueryResult(AggContext *aggContext, std::string &query)
{
...
void WenChuangSearchProcessor::ReRankByHost(std::vector<RankNode>& rankVec)
{
    // host过滤
    AggDataManager * aggDataManager = AggDataManager::GetInstance();
    AggDataWrapper filterHostWrapper(aggDataManager, "wenchuang_filter_host_data");
    AggDataWrapper goodHostWrapper(aggDataManager, "wenchuang_good_host_data");
    if (filterHostWrapper.Get<ValueData>() == NULL ||
            goodHostWrapper.Get<ValueData>() == NULL) {
        AGG_LOG(ERROR, "Get Wenchuang Filter/Good Host failed.");
        return;
    }
...
HttpResponsePtr WenChuangSearchProcessor::AsyncRequestAgg(AggContext *aggContext, std::string query)
{
...
HttpResponsePtr WenChuangSearchProcessor::AsyncRequestCommon(AggContext *aggContext, std::string query, const std::string& type)
{
...
void WenChuangSearchProcessor::RequestAggFormatResult(AGG::AggContext *aggContext, AggWebResultPtr aggResultPtr)
{
    std::string uid;
    ...
    if (aggResultPtr) {
        if (! aggResultPtr->mDocPtrVec.empty()) {
            ...
            int doc = 0;
            for(size_t i = 0; i < docPtrVec.size() && i < size_t(6); ++i) {
...
void WenChuangSearchProcessor::GetDebugInfoPostfix(AggWebResultPtr aggResultPtr)
{
    if (! aggResultPtr) {
        return;
    }
...
BOOST_CLASS_EXPORT_IMPLEMENT(AGG_NS(medical)::WenChuangSearchProcessor)
