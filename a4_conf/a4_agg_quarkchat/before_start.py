...
searchAppName = os.environ.get("SEARCH_APP_NAME")
if not searchAppName:
    searchAppName = 'a4_agg_web'

appName = searchAppName[3:]

TEMPLATEFILE_PREFIX = "/a4/usr/local/conf/"+searchAppName
...
def convert(path):
    ...
    if ENV == "test" or ENV == "pub" or ENV == "perf":
        kvs['deploy_env'] = searchAppName + '/' + ENV
    elif ENV == "online_em21":
        kvs['deploy_env'] = searchAppName + '/em21'
    elif ENV == "online_na61":
        kvs['deploy_env'] = searchAppName + '/na61'
    try:
...
if __name__ == "__main__":
    ...
    downloadData('/a4/usr/local/conf/'+searchAppName+'/'+appName+'_data.conf')
    replaceAlogConf()
    startLogCleaner()
